  ...
  if (!exists(x = "map", envir = .RnightlightsEnv))
  {
    saveMapPath <-
      file.path(
        ...
        paste0(
          "rworldmap_",
          utils::packageVersion("rworldmap"),
          "_map.rda"
        )
      )
                              ...
                              downloadMethod = pkgOptions("downloadMethod"))
{
  ...
  if (gadmVersion == "2.8")
    baseUrl <-
      paste0("https://biogeo.ucdavis.edu/data/gadm",
             gadmVersion,
             "/rds/")
  else
    baseUrl <-
  ...
  while (TRUE)
  {
    ...
    #Create an integer col in the shapefile corresponding to the unique GIDs
    if (gadmVersion == "3.6" &&
        is.null(ctryPoly@data[[lowestIDCol]]))
    {
      message(Sys.time(),
              ": Creating integer zone attribute col for polygon")
  ...
  if (!file.exists(
    getCtryStructFnamePath(ctryCode,
                           gadmVersion = gadmVersion,
                           gadmPolyType = gadmPolyType)
  ))
                           ...
                           custPolyPath = NULL)
{
  ...
  if (is.null(custPolyPath))
  {
    ...
    ctryPolyUrl <- if (gadmVersion == "2.8")
      paste0("https://biogeo.ucdavis.edu/data/gadm2.8/shp/",
             ctryCode,
             "_adm_shp.zip")
    else if (gadmVersion == "3.6")
      paste0("https://biogeo.ucdavis.edu/data/gadm3.6/shp/gadm36_",
             ctryCode,
             "_shp.zip")
    else
