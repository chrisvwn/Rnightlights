...
shiny::shinyServer(function(input, output, session){
  ...
  output$countries <- shiny::renderUI({
    ...
    diffCtryCodes <- allCtryCodes[!(allCtryCodes %in% ctryCodesWithData)]
  ...
  ######################## reactive ctryNlData ###################################
    ...
    ctryNlData <- shiny::reactive({
      ...
      nlPeriod <- Rnightlights::dateToNlPeriod(dt = nlPeriod, nlType = nlType)      

      if(length(nlPeriod) == 2)
        nlPeriod <- Rnightlights::nlRange(startNlPeriod = nlPeriod[1], endNlPeriod = nlPeriod[2], nlType = nlType)
      ...
      multiTileStrategy <- shiny::isolate(input$multiTileMergeStrategy)
      ...
      if (length(countries) == 1)
      {
        ...
        lyrNum <- which(unlist(ctryAdmLevels()) == shiny::isolate(admLevel))-1
      ...
      } else if(length(countries) > 1) #remove subcountry admin levels
      {
        for (ctryCode in countries)
        {
          ...
          temp <- Rnightlights:::getCtryNlData(ctryCode = ctryCode,
                                               admLevel = admLevel,
                                               gadmVersion = polyVer,
                                               gadmPolyType = polyType,
                                               custPolyPath = custPolyPath,
  ...
  output$polySrc <- shiny::renderUI({
    ...
    if(!is.null(countries))
      existingPolySrcs <- existingPolySrcs[existingPolySrcs$ctryCode %in% countries, ]

    admLevel <- input$radioAdmLevel
    
    if(!is.null(admLevel))
      existingPolySrcs <- existingPolySrcs[existingPolySrcs$admLevel == admLevel, "polySrc"]
    else
    ...
    polySrcs <- list("Existing"=existingPolySrcs, "Process"=polySrcs)
  ...
  output$polyVer <- shiny::renderUI({
    ...
    if(!is.null(countries))
      existingPolyVers <- existingPolyVers[existingPolyVers$ctryCode %in% countries, ]
    
    admLevel <- input$radioAdmLevel
    
    if(!is.null(admLevel))
      existingPolyVers <- existingPolyVers[existingPolyVers$admLevel == admLevel, "polyVer"]
    else
    ...
    polyVers <- list("Existing"=existingPolyVers, "Process"=polyVers)
  ...
  output$polyType <- shiny::renderUI({
    ...
    if(!is.null(countries))
      existingPolyTypes <- existingPolyTypes[existingPolyTypes$ctryCode %in% countries, ]
    
    admLevel <- input$radioAdmLevel
    
    if(!is.null(admLevel))
      existingPolyTypes <- unique(existingPolyTypes[existingPolyTypes$admLevel == admLevel, "polyType"])
    else
      existingPolyTypes <- unique(existingPolyTypes[, "polyType"])
    
    polyTypes <- allPolyTypes[!(allPolyTypes %in% existingPolyTypes)]
    ...
    polyTypes <- list("Existing"=existingPolyTypes, "Process"=polyTypes)
  ...
  ######################## renderUI ctryStats ###################################
    ...
    output$ctryStats <- shiny::renderUI({
      ...
      #memory
      if(!is.null(ctryStat) && ctryStat != "" && ctryStat %in% c(existingStats))
      {
  ...
  output$configName <- shiny::renderUI({
    ...
    allConfigNames <- toupper(allConfigNames[allConfigNames$nlType == nlType, "configName"])
    ...
    configNames <- list("Existing"=existingConfigNames, "Process"=allConfigNames)
    
    if(length(existingConfigNames) == 0)
      selectedConfigName <- toupper(Rnightlights::pkgOptions(paste0("configName_", nlType)))
    else
  ...
  output$multiTileMergeStrategy <- shiny::renderUI({
    ...
    if(!is.null(countries))
      existingMultiTileMergeStrategys <- existingMultiTileMergeStrategys[existingMultiTileMergeStrategys$ctryCode %in% countries, ]
    
    admLevel <- input$radioAdmLevel
    
    if(!is.null(admLevel))
      existingMultiTileMergeStrategys <- existingMultiTileMergeStrategys[existingMultiTileMergeStrategys$admLevel == admLevel, "multiTileMergeStrategy"]
    else
      existingMultiTileMergeStrategys <- unique(existingMultiTileMergeStrategys[, "multiTileMergeStrategy"])
    
    multiTileMergeStrategys <- allMultiTileMergeStrategys[!(allMultiTileMergeStrategys %in% existingMultiTileMergeStrategys)]
    
    if(length(existingMultiTileMergeStrategys) == 1)
      existingMultiTileMergeStrategys <- list(existingMultiTileMergeStrategys)
    
    if(length(multiTileMergeStrategys) == 1)
      multiTileMergeStrategys <- list(multiTileMergeStrategys)
    
    multiTileMergeStrategys <- list("Existing"=existingMultiTileMergeStrategys, "Process"=multiTileMergeStrategys)
    
    if(length(existingMultiTileMergeStrategys) == 0)
      selectedMultiTileMergeStrategy <- Rnightlights::pkgOptions("multiTileStrategy")
    else
  ...
  output$multiTileMergeFun <- shiny::renderUI({
    ...
    if(!is.null(countries))
      existingMultiTileMergeFuns <- existingMultiTileMergeFuns[existingMultiTileMergeFuns$ctryCode %in% countries, ]
    
    admLevel <- input$radioAdmLevel
    
    if(!is.null(admLevel))
      existingMultiTileMergeFuns <- existingMultiTileMergeFuns[existingMultiTileMergeFuns$admLevel == admLevel, "multiTileMergeFun"]
    else
      existingMultiTileMergeFuns <- unique(existingMultiTileMergeFuns[, "multiTileMergeFun"])
    
    multiTileMergeFuns <- allMultiTileMergeFuns[!(allMultiTileMergeFuns %in% existingMultiTileMergeFuns)]
    ...
    multiTileMergeFuns <- list("Existing"=existingMultiTileMergeFuns, "Process"=multiTileMergeFuns)
    
    if(length(existingMultiTileMergeFuns) == 0)
      selectedMultiTileMergeFun <- Rnightlights::pkgOptions("multiTileMergeFun")
    else
  ...
  output$removeGasFlares <- shiny::renderUI({
    ...
    if(!is.null(countries))
      existingRemoveGasFlares <- existingRemoveGasFlares[existingRemoveGasFlares$ctryCode %in% countries, ]
    
    admLevel <- input$radioAdmLevel
    
    if(!is.null(admLevel))
      existingRemoveGasFlares <- existingRemoveGasFlares[existingRemoveGasFlares$admLevel == admLevel, "removeGasFlares"]
    else
      existingRemoveGasFlares <- unique(existingRemoveGasFlares[, "removeGasFlares"])
    
    removeGasFlares <- allRemoveGasFlares[!(allRemoveGasFlares %in% existingRemoveGasFlares)]
    ...
    removeGasFlares <- list("Existing"=existingRemoveGasFlares, "Process"=removeGasFlares)
    
    if(length(existingRemoveGasFlares) == 0)
      selectedRemoveGasFlare <- Rnightlights::pkgOptions("removeGasFlares")
    else
  ...
  observe({
    ...
    polyVer <- input$polyVer
    polyType <- input$polyType
    custPolyPath <- if(!is.null(polySrc) && polySrc == "CUST") polyVer else NULL
  ...
  observeEvent(input$countries, {
    if(length(input$countries) > 1)
       updateRadioButtons(session = session, inputId = "admLevel", selected = "country")
  })
  
  observe({
    print("here")
    
    if((length(input$countries) > 0) && !is.null(input$radioAdmLevel) && is.null(ctryNlDataList()))
    {
  ...
  observeEvent(input$newStat, {
    ...
    updateTextAreaInput(session = session, inputId = "newStat", label = "Function Name/Definition")
  })
  ...
  observeEvent(input$btnNewStat, {
    ...
    if(validStat)
    {
      ...
      if(!is.null(ctryNlDataList))
      {
        ...
        if(newStat %in% existingStats)
          updateTextAreaInput(session = session, inputId = "newStat", label = "Function name exists")
        else
        {
          updateTextAreaInput(session = session, inputId = "newStat", label = "Function OK")
          ...
          if(!is.null(input$countries))
          {
            ...
            if(exists("newStatBody"))
              values$newStatFuncBody <- eval(parse(text=newStatBody))
            else
    ...
    {
      updateTextAreaInput(session = session, inputId = "newStat", label = "Invalid Function")
  ...
  ######################## renderUI radioAdmLevel ###################################
    ...
    output$radioAdmLevel <- shiny::renderUI({
      ...
      admLevels <- unlist(sapply(1:length(admLevels), function(admLevel)
      {
        ...
        if(!is.null(availData))
          admLevelHasData <- nrow(availData[availData$ctryCode == countries &
                                                   availData$admLevel == paste0("ADM", admLevel-1),]) > 0
        else
      ...
      shiny::radioButtons(inputId = "radioAdmLevel", 
                     label = "Admin Level", 
    ...
    output$selectAdmLevel <- shiny::renderUI({
      ...
      if((length(countries) != 1 || identical(countries, character(0)) || grepl("^\\s*$", countries)))
        return()
      ...
      if (ncol(ctryAdmLevelNames)>2)
        elems <- lapply(2:length(ctryAdmLevels), function(lvlIdx){
          ...
          lvlSelect <- dplyr::group_by(lvlSelect, ctryAdmLevels[lvlIdx-1])
    ...
    selectedAdmLevel3 <- shiny::reactive({
      ...
      x <- Rnightlights::searchAdmLevel(ctryCodes = input$countries, admLevelNames = x)
    ...
    shiny::observe({
      ...
      updateRadioButtons(session = session, inputId = "radioAdmLevel", selected = ctryAdmLevels[as.numeric(lowestSelected)])
...
#          }
          ...
          if(length(input[[paste0("selectAdm",lvlIdx-1)]]) == 1)
          {
            ...
            shiny::updateSelectizeInput(session, paste0("selectAdm", lvlIdx), choices = lvlSelect, selected = input[[paste0("selectAdm", lvlIdx)]][input[[paste0("selectAdm", lvlIdx)]] %in% unlist(lvlSelect)])
          }
  ...
  #})
    ...
    output$sliderNlPeriodRange <- shiny::renderUI({
      ...
      shiny::isolate({
        ...
        } else #if (is.null(ctryData))
        {
          ...
          if(!is.null(nlP))
            nlP <- Rnightlights::dateToNlPeriod(dt = nlP, nlType = nlType)
          
          if(!is.null(nlP) && !all(Rnightlights::validNlPeriods(nlPeriods = nlP, nlTypes = nlType)))
            nlPeriodRange <- NULL
          ...
          } else if(!is.null(nlPeriodRange))
          {
            ...
            endDate <- Rnightlights::dateToNlPeriod(nlPeriodRange[2], nlType)
          } else {
            if(is.null(nlType))
              nlType <- "VIIRS.M"
        ...
        allNlPeriods <- unlist(Rnightlights::getAllNlPeriods(nlTypes = nlType))
        minDate <- Rnightlights::nlPeriodToDate(nlPeriod = allNlPeriods[1], nlType = nlType)
        maxDate <- Rnightlights::nlPeriodToDate(nlPeriod = allNlPeriods[length(allNlPeriods)], nlType = nlType)
    ...
    output$sliderNlPeriod <- shiny::renderUI({
      ...
      #shiny::isolate({
        ...
        if(!is.null(values$lastUpdated) && values$lastUpdated == "nlPeriod")
        {
          value <- nlPeriod
        } else #if (is.null(ctryData))
        {
          ...
          value <- gsub("-*$", "", paste(substr(value, 1,4), substr(value, 5,6),substr(value, 7,8), sep = "-"))
    ...
    hCluster <- shiny::reactive({
      ...
      shiny::isolate({
        ...
        if (is.null(countries) || is.null(meltCtryData) || nrow(meltCtryData) < 3)
          return()
    ...
    output$plotHCluster <- shiny::renderPlot({
      ...
      shiny::isolate({
        ...
        dendro %>%
          dendextend::color_branches(k = numClusters, col = rev(cbPalette[1:numClusters]), groupLabels = T) %>%
      ...
      shiny::isolate({      
        ...
        for(country in countries[2:length(countries)])
        {
          ...
          mapLabels <- sprintf(
            ...
            lvlCtryData[[1]],
            cutClusts,
            format(lvlCtryData[[2]],scientific = T,digits = 2),
            lvlCtryData[["rank"]],
            nrow(lvlCtryData)
        ...
        {
          map <- map %>% leaflet::addPolygons(data=ctryPoly0,
                                              layerId = countries,
                                              fill = FALSE,
                                              fillColor = "#fefe40",
        ...
        if (is.null(countries) || is.null(ctryData))
          ...
          if (length(countries)==1)
          {
            ...
            g <- ggplot2::ggplot(ctryData,
                                 aes(x=eval(parse(text=aggNames[length(aggNames)])),
                                     y=value, col=year, group=year)) +
              ggplot2::geom_line(alpha=0.3) + ggplot2::geom_point() +
              ggplot2::geom_smooth(aes(group=1),
                                   method = "loess",
                                   weight=1,
                                   alpha=0.2,
                                   lty='twodash') #+ theme(axis.text.x=element_text(angle=90,hjust=1,vjust=0.5)) #+ labs(col=year)
          }
          else
          {
            #ctryData <- aggregate(value ~ country+variable, data=ctryData, mean)
            #switched to data.table aggregation
            ctryData <- stats::setNames(ctryData[,mean(value, na.rm = TRUE),by = eval(parse(text=lstAggBy))],
                                        c(admLevel, "variable", aggNames, "value"))
            #g <- ggplot2::ggplot(data=ctryData, aes(x=variable, y=value, col=country, group=year))
            g <- ggplot2::ggplot(ctryData, aes(x=eval(parse(text=aggNames[length(aggNames)])),
                                               y=value, col=country,
                                               shape=year,
                                               group=interaction(country,year))) +
              ggplot2::geom_line(lwd=.5, alpha=0.3) +
              ggplot2::geom_point()+
              ggplot2::geom_smooth(aes(group=country),
                                   method = "loess",
                                   weight=1,
                                   alpha=0.2,
                                   lty='twodash')
    ...
    output$plotNightLights <- plotly::renderPlotly({
      ...
      shiny::isolate({
        ...
        admLvlNums <- as.numeric(gsub(pattern = "[^[:digit:]]", replacement = "", x = admLvlNums))
        ...
        else if (graphType == "line")
        {
          if (length(countries)==1)
          {
            #switched to data.table aggregation
            #ctryData <- stats::setNames(aggregate(ctryData$value, by=list(ctryData[,admLevel], ctryData[,"variable"]), mean, na.rm=T), c(admLevel, "variable", "value"))
            ctryData <- stats::setNames(ctryData[,value,by = list(ctryData[[admLevel]], variable)], c(admLevel, "variable", "value"))
          ...
          else
          {
            #ctryData <- aggregate(value ~ country+variable, data=ctryData, mean)
            #switched to data.table aggregation
            ctryData <- stats::setNames(ctryData[,value,by = list(country, variable)], c("country", "variable", "value"))
        ...
        else if (graphType == "histogram")
        {
          ...
          g <- g + ggplot2::geom_histogram(aes(y=..density..),
                                           bins = 30,
                                           colour="black",
                                           fill="white") +
    ...
    output$dataset <- DT::renderDataTable({
      ...
      startDate <- Rnightlights::dateToNlPeriod(input$nlPeriodRange[1], input$nlType)
      
      endDate <- Rnightlights::dateToNlPeriod(input$nlPeriodRange[2], input$nlType)
      ...
      #the cols with the stats we want
      dataCols <- grep(pattern = ctryStat, x = allCols, value = TRUE, fixed = T)
      
      dataCols <- grep(pattern = configName, x = dataCols, value = TRUE)
      
      dataCols <- grep(pattern = paste0("GF", substr(removeGasFlares, 1, 1)), x = dataCols, value = TRUE)
      
      dataCols <- grep(pattern = paste0("MTS", multiTileMergeStrategy), x = dataCols, value = TRUE)
      
      dataCols <- grep(pattern = multiTileMergeFun, x = dataCols, value = TRUE)
      
      dataColDates <- gsub(".*_(\\d{4,8})_.*", "\\1", dataCols)
      
      dataCols <- dataCols[dataColDates >= startDate & dataColDates <= endDate]
    ...
    observe({
      ...
      if(is.null(countries) || is.null(input$nlPeriod) || is.null(input$nlType))
        return()
      ...
      if(length(countries) > 1){
        ...
        if(length(selected)>0)
        {
          ...
          map <- leaflet::flyToBounds(map = map, lng1 = mapExtent@xmin, lat1 = mapExtent@ymin, lng2 = mapExtent@xmax, lat2 = mapExtent@ymax)
        }
        else
          map <- map %>% leaflet::addLayersControl(overlayGroups = c("ctryRaster",
                                                                     ctryAdmLevels[2:lyrNum]))
        ...
        if (exists("mapExtent"))
          map <- map %>% 
        leaflet::flyToBounds(mapExtent@xmin, mapExtent@ymin, mapExtent@xmax, mapExtent@ymax)
